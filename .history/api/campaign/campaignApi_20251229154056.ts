...
export const generateAIImageApi = async (
  ...
  try {
    ...
    // If API returned imagePrompt, convert it to an actual image URL
    const images: string[] = apiData.imagePrompt
      ? [`https://image.pollinations.ai/prompt/${encodeURIComponent(data.prompt)}?nologo=true&seed=${Math.floor(Math.random() * 1000000)}`]
      : [];
