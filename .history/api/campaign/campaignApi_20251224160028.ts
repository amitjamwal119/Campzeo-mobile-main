...
import { getCampaignsApi } from "@/api/campaign/campaignApi"; // make sure you have this API
...
export default function CreateContact() {
  ...
  useEffect(() => {
    const fetchCampaigns = async () => {
      ...
      try {
        ...
        setCampaignOptions(data?.data ?? []); // adjust based on your API response
      } catch (err) {
  ...
  const onSubmit = async (data: Contact) => {
    try {
      ...
      if (isEdit) {
        const updated = await updateContactApi(Number(contactId), data, token);
        Alert.alert("Success", "Contact updated successfully");
      } else {
        const created = await createContactApi(data, token);
