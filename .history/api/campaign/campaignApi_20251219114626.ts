...
// Generate AI message
export const generateAIMessageApi = async (subject: string, platform: string, token?: AuthToken) => {
  try {
    const apiKey = token || process.env.OPENAI_API_KEY; // You can store OpenAI key in env
...
// Generate AI image
export const generateAIImageApi = async (subject: string, platform: string, token?: AuthToken) => {
  try {
...
2️⃣ Update CampaignPostForm.tsx to call API from the API file
