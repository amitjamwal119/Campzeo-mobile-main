...
// Get single post by campaignId & postId (for edit)
export const getPostByIdApi = async (
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/campaigns/${campaignId}/posts/${postId}/edit`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
    ...
    console.error("Get Post By ID API Error:", error.response || error.message);
    throw error;
  }
};
