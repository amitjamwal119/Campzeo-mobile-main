...
export default function Notifications() {
  ...
  const grouped = filtered.reduce((acc: any, item) => { acc[item.date] = acc[item.date] || []; acc[item.date].push(item); return acc; }, {});
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#EEF2FF" }}>
      <ThemedView className="flex-1 px-4">
        ...
        <View className="flex-row items-center justify-between my-5">
          ...
          {/* Right icon placeholder for alignment */}
          <View style={{ width: 32 }} />
        </View>
        ...
        {hasNotifications && (
          <SectionList
            ...
            renderSectionHeader={({ section: { title } }) => (
              <Text className="text-gray-500 font-medium mb-2">{title}</Text>
            )}
          />
        )}
      </ThemedView>
...
export default function Notifications() {
  ...
  // Group notifications by date and attach index for header control
  ...
  const sections = Object.keys(grouped).map((date, index) => ({
    title: date,
    data: grouped[date],
    index, // used to hide first section header
  }));
