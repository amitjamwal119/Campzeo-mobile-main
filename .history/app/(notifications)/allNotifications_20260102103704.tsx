...
// ✅ TEMP: replace this with your real auth token source
const AUTH_TOKEN = ""; // <-- paste token here if needed
...
export default function AllNotifications() {
  ...
  // ---------------- FORMAT API DATA ----------------
  const formatNotification = (item: any): NotificationItem => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const res = await getNotificationsApi(AUTH_TOKEN, 1, 5);

      const formatted =
        res?.data?.notifications?.map(formatNotification) || [];
