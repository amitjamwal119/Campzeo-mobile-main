    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const res = await deleteNotificationApi(token, id);

              // If API returns success or message says already deleted
              if (res?.isSuccess || res?.message === "Notification already deleted") {
                setNotifications((prev) => prev.filter((n) => n.id !== id));
              } else {
                Alert.alert("Error", res?.message || "Failed to delete notification");
              }
            } catch (error: any) {
              console.log("Delete Notification API Error:", error.response || error.message);

              // Remove locally if already deleted
              if (error?.response?.data?.message === "Notification already deleted") {
                setNotifications((prev) => prev.filter((n) => n.id !== id));
              } else {
        ...
        <View className="flex-row items-center mb-3">
          <View className="flex-1 flex-row items-center bg-white rounded-full px-4 border border-gray-300 shadow-sm">
            ...
            <TextInput
              ...
              onChangeText={(v) => setVisibleCount(5)}
            />
          </View>
            ...
            renderItem={({ item }) => (
              <View className="flex-row justify-between items-center">
                <TouchableOpacity
                  ...
                  className={`flex-1 p-4 mb-2 rounded-2xl shadow-sm ${
