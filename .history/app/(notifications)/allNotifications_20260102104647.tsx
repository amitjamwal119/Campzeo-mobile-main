...
import { useAuth } from "@clerk/clerk-expo"; // 👈 get auth token
...
export default function AllNotifications() {
  ...
  // Format API data for SectionList
...
const fetchNotifications = async () => {
  try {
    ...
    const token = await getToken(); // Replace with your actual token getter
    if (!token) {
    ...
    const notificationsArray = Array.isArray(res) ? res : res.notifications || [];
    const formatted = notificationsArray.map(formatNotification);
      ...
      <ThemedView className="flex-1 px-4 pt-4">
        ...
        <View className="flex-row items-center justify-between mb-3">
          <View className="flex-row space-x-2">
            ...
            >
              <Text className="font-semibold" style={{ color: tab === "All" ? "#fff" : "#374151" }}>
            ...
            >
              <Text className="font-semibold" style={{ color: tab === "Unread" ? "#fff" : "#374151" }}>
                Unread ({unreadCount})
              </Text>
            </TouchableOpacity>
          </View>
