...
const handleExportAll = async () => {
  try {
    ...
    // 1️⃣ Fetch contacts as ArrayBuffer
    const arrayBuffer = await exportContactsApi(token); // must return ArrayBuffer

    if (!arrayBuffer) throw new Error("No contacts to export");

    // 2️⃣ Convert ArrayBuffer → UTF-8 string
    const decoder = new TextDecoder("utf-8");
    const csvContent = decoder.decode(arrayBuffer);

    // 3️⃣ Write CSV to file
