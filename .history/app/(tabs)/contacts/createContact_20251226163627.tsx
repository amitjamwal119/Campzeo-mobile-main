      ...
      <TouchableOpacity
        onPress={() => {
          if (selectedCampaigns.length === campaignOptions.length) {
            ...
            // Select all
            setValue("campaignIds", campaignOptions.map((c) => c.id));
          }
        }}
        className="mb-3 px-4 py-2 rounded bg-red-500"
      >
        <Text className="text-white font-semibold text-center">
          {selectedCampaigns.length === campaignOptions.length ? "Unselect All" : "Select All"}
        </Text>
      </TouchableOpacity>
