...
export interface ContactsRecord {
  ...
  whatsapp: string;
  show?: boolean;
  createdDate?: string; // Added created date
    ...
    >
      ...
      {record.show && (
        <View className="mt-3 space-y-2">
          {/* Email, Mobile, WhatsApp, Created Date */}
          {[
            { label: "Email", value: record.email, icon: "mail-outline", color: "6b7280" },
            { label: "Mobile", value: record.mobile, icon: "call-outline", color: "6b7280" },
            { label: "WhatsApp", value: record.whatsapp, icon: "logo-whatsapp", color: "6b7280" },
            { label: "Created Date", value: record.createdDate ?? "N/A", icon: "calendar-outline", color: "6b7280" },
          ].map((item, index) => (
