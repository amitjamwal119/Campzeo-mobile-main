...
export default function Contacts() {
  ...
  const handleExportAll = async () => {
    try {
      ...
      const blob = await exportContactsApi(token);
      ...
      const file = await File.createAsync(
        Directory.cache,
        `contacts_export_${Date.now()}.csv`
      );

      await file.writeAsync(base64Data, { encoding: "base64" });
