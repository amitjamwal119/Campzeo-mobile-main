...
const handleExportAll = async () => {
  try {
    ...
    // 1️⃣ Fetch CSV content from API
    // Make sure your API returns text/csv as string
    const res = await exportContactsApi(token);
    const csvContent: string = res?.csv || res; // adjust based on your API response

    if (!csvContent) throw new Error("No contacts to export");

    // 2️⃣ Write CSV to file
    ...
    // 3️⃣ Share / Download the file
    const canShare = await Sharing.isAvailableAsync();
    if (canShare) {
      await Sharing.shareAsync(fileUri, { mimeType: "text/csv" });
    } else {
      Alert.alert("Exported", `Contacts exported to: ${fileUri}`);
    }
  } catch (e: any) {
