...
const handleGenerateImage = async () => {
  ...
  try {
    ...
    const imagesArray = response.images || response.data?.map((i: any) => i.url);

    if (imagesArray && imagesArray.length > 0) {
      setGeneratedImages(imagesArray);
      setSelectedImage(imagesArray[0]);
    } else {
      Alert.alert("No images returned", "Check your API response or prompt");
      console.warn("No images returned:", response);
    }
  } catch (error: any) {
    Alert.alert("Image Generation Error", error?.message || "Failed to generate image");
  } finally {
    setLoadingImage(false); // keep modal open
  }
};
