...
export default function CampaignPostForm({
  ...
  const handleGenerateText = () => {
    if (!subject) {
      Alert.alert("Please enter subject first");
      return;
    }

    setMessage(`This is an auto-generated message for: "${subject}"`);
  };
    ...
    setMessage(`This is an auto-generated Image for: "${subject}"`);
  };
    ...
    >
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <View className="flex-1 bg-gray-100 p-4">
          ...
          {/* ✅ GENERATE TEXT BUTTON (ONLY ADDITION) */}
          <TouchableOpacity
            onPress={handleGenerateText}
            className="w-full flex-row items-center justify-center bg-[#dc2626] px-4 py-3 rounded-full mb-4"
          >
            <Ionicons
              name="sparkles"
              size={20}
  ...
  <View className="absolute inset-0 bg-black/50 items-center justify-center z-50">
    <View className="bg-white w-[90%] rounded-2xl p-4">
      ...
      <View className="flex-row items-center border border-gray-300 rounded-full px-3">
        <TextInput
          placeholder="enter text or anything according to you"
          value={aiPrompt}
        ...
        <TouchableOpacity
          onPress={handleGenerateText}
          disabled={loadingAI}
          className="bg-[#dc2626] px-4 py-2 rounded-full ml-2"
        >
