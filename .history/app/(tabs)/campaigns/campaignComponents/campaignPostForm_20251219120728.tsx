...
export default function CampaignPostForm({
  ...
  const handleAIGenerateMessage = async () => {
    if (!subject) {
      Alert.alert("Enter a subject first to generate AI message!");
      return;
    }
    try {
      setLoadingAI(true);
      const token = await getToken();
      const aiMessage = await generateAIMessageApi(subject, platform, token);
      setMessage(aiMessage ?? undefined);
    } catch (err) {
  ...
  const handleAIGenerateImage = async () => {
    if (!subject) {
      Alert.alert("Enter a subject first to generate AI image!");
      return;
    }
    try {
      setLoadingAI(true);
      const token = await getToken();
      const imageUrl = await generateAIImageApi(subject, platform, token);
            ...
            <>
              <View className="flex-row items-center mb-4">
                <TextInput
                  ...
                  value={message ?? undefined} // fixed null | undefined
              ...
              <View className="flex-row items-center mb-4">
                <TextInput
                  placeholder="Generated AI Image"
                  value={generatedImage ?? undefined} // fixed null | undefined
                  className="flex-1 border border-gray-300 border-r-0 rounded-l-full px-3 h-12 bg-white"
                  editable={false}
                />
                <TouchableOpacity
                ...
                >
                  {loadingAI ? <ActivityIndicator color="#fff" /> : <Ionicons name="sparkles" size={24} color="#fff" />}
                </TouchableOpacity>
              </View>
