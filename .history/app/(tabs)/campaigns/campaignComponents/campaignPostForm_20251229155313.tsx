...
const response = await generateAIImageApi({ prompt: imagePrompt }, token);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      // Pollinations returns 'imagePrompt' (not a real image) → use placeholder for demo
      const imageUrl = response?.imagePrompt;

      // You must replace this with a real image generation API to get direct image URLs
      // For now, fallback to placeholder
      const validImageUrl = imageUrl || "https://picsum.photos/200";

      setGeneratedImages([validImageUrl]);
      setSelectedImage(validImageUrl);
    } catch (error: any) {
