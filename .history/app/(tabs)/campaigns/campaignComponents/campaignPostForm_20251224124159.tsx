...
export default function CampaignPostForm({
  ...
  onClose,
  onCreatedNavigate,
  existingPost, // <-- new prop for editing
}: {
  platform: string;
  campaignId?: string;
  onClose?: (newPost?: any) => void;
  onCreatedNavigate?: () => void;
  existingPost?: any; // optional post data for editing
}) {
  const { getToken } = useAuth();

  // Initialize state from existingPost if provided
...
/* GENERATED IMAGE SELECTION (HIDDEN UNTIL GENERATE) */
{generatedImages.length > 0 && !selectedImage && setSelectedImage(generatedImages[0])}

/* SELECTED IMAGE BOX (SHOW ALWAYS AFTER GENERATION) */
{selectedImage && (
