...
const handleGenerateImage = async () => {
  ...
  try {
    ...
    // Call your AI image API
    const response = await generateAIImageApi({ prompt: imagePrompt }, token);

    console.log("AI Image API Response:", response);

    // FIX: Use a real image URL from your API response
    // Replace this with the actual property that contains the real image URL
    // For example, if your API returns { imageUrl: "https://..." }
    const imageUrl = response?.imageUrl || response?.image; // <- use real image URL here

    if (!imageUrl) throw new Error("No valid image URL returned");
    ...
    setGeneratedImages(["https://via.placeholder.com/100"]);
    setSelectedImage("https://via.placeholder.com/100");
  } finally {
  ...
  resizeMode="cover"
  onError={() => console.log("Image failed to load", selectedImage)}
/>
