...
export default function CampaignPost() {
    ...
    const fetchPostDetails = async () => {
      try {
        ...
        if (isMounted && data?.post) {
          setExistingPost(data.post); // update existing post state
          setSelected(data.post.type || type || null); // select correct social media type
        }
      } catch (error: any) {
        console.error("Error fetching post details:", error.response || error.message);
        Alert.alert("Error", "Failed to fetch post details. Please try again.");
      }
    };
  ...
  }, [campaignId, postId, type, getToken]);
        ...
        {selected && (
          <View style={{ marginTop: 0, marginBottom: 5 }}>
            <CampaignPostForm
              ...
              existingPost={existingPost} // ✅ Pass existing post for prefilling
              onClose={() => {
                setSelected(null);
