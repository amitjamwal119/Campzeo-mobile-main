...
export default function CampaignCard({
  ...
  <View style={{ position: "absolute", right: 0, top: 0, flexDirection: "row" }}>
    ...
    {postButtonTopRight && showPostButton && (
      <TouchableOpacity onPress={handleAddPost} style={{ marginRight: 8 }}>
    ...
    {showActions && (
      <>
        <TouchableOpacity onPress={handleEdit} style={{ marginHorizontal: 4 }}>
          <Ionicons name="create-outline" size={22} color="#10b981" />
        </TouchableOpacity>

        <TouchableOpacity onPress={handleDelete} style={{ marginHorizontal: 4 }}>
          <Ionicons name="trash-outline" size={22} color="#ef4444" />
        </TouchableOpacity>

        <TouchableOpacity onPress={() => onCopy(campaign)} style={{ marginHorizontal: 4 }}>
          <Ionicons name="copy-outline" size={22} color="#3b82f6" />
        </TouchableOpacity>

        <TouchableOpacity onPress={() => onToggleShow(campaign)} style={{ marginHorizontal: 4 }}>
          <Ionicons
