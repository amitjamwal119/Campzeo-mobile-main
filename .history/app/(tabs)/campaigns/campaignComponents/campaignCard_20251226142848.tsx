...
export default function CampaignCard({
  ...
  const postsCount = campaign.posts?.length ?? 0;
    ...
    <View className="bg-gray-200 p-4 rounded-xl mb-4 shadow">
      ...
      {isExpanded && (
        <View>
          ...
          <View className="mb-3">
            {/* Row 1: Duration heading + Status */}
            <View className="flex-row justify-between items-center mb-2">
              <Text className="font-bold text-gray-900">Duration</Text>

              <View
                className={`px-2.5 py-1 rounded-full ${statusStyles[status]?.bg ?? "bg-gray-200"
                  }`}
              >
                <Text
                  className={`text-[12px] font-semibold ${statusStyles[status]?.text ?? "text-gray-700"
                    }`}
                >
                  {status}
                </Text>
            ...
            <View className="flex-row justify-between items-center mt-3 w-full">
              {/* Contacts Count */}
              <View className="flex-row items-center">
                <Ionicons name="people-outline" size={18} color="#4b5563" />
                <Text className="ml-1.5 text-gray-700 text-base">
