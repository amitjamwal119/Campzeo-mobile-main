...
const handleAIGenerateMessage = async () => {
  ...
  try {
    ...
    if (!response?.content) throw new Error("Invalid AI response");

    // ✅ Store multiple suggestions in an array
    const suggestions = [
      response.content,
      ...(response.variations?.map((v) => v.content) ?? [])
    ];

    setAiResults(suggestions); // array of suggestions
    setModalVisible(true); // open popup with scrollable list
  } catch (error: any) {
