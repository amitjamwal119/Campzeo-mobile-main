...
const handleCustomThumbnailUpload = async () => {
  try {
    ...
    // Explicitly type the asset
    const file: { uri: string; name: string; mimeType?: string } | undefined = result.assets?.[0];

    if (file) {
      setCustomThumbnail(file.uri);
    }
  } catch (error) {
    ...
    >
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <View className="flex-1 bg-gray-100">
          ...
          {platform === "YOUTUBE" && (
  <View className="mt-6 bg-gray-100 p-4 rounded-lg shadow-md">

    {/* ---------- YouTube Settings Heading ---------- */}
    <Text className="text-lg font-bold mb-4">YouTube Settings</Text>

    {/* ---------- Content Type ---------- */}
    <Text className="font-semibold mb-2">Content Type</Text>
      ...
      {["Standard Video", "YouTube Short", "Playlist"].map((type) => (
        <TouchableOpacity
          ...
          style={{
            ...
            marginHorizontal: 4,
            borderRadius: 8,
            backgroundColor: youTubeContentType === type ? "#2563eb" : "#f3f3f3",
            alignItems: "center",
          }}
        >
          <Text style={{ color: youTubeContentType === type ? "#fff" : "#000", fontWeight: "bold" }}>
            {type}
          </Text>
        </TouchableOpacity>
      ))}
