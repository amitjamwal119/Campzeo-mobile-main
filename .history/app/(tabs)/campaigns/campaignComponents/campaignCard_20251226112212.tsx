...
export default function CampaignCard({
  ...
  const getStatus = () => {
    ...
    const dateParts = campaign.dates.split("-");
    if (dateParts.length < 2) return "Scheduled";

    const endDateStr = dateParts[1]?.trim();
    if (!endDateStr) return "Scheduled";

    const endDate = new Date(endDateStr);
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    return endDate < today ? "Completed" : "Scheduled";
  };
  ...
  const contactsCount =
    campaign.contactsCount ?? (Array.isArray(campaign.contacts) ? campaign.contacts.length : 0);
          ...
          <View className="mb-3">
            ...
            <View className="flex-row items-center mt-3 w-full">
              ...
              <View className="flex-1 items-center">
                <View
                  className={`px-2.5 py-1 rounded-full ${status === "Completed" ? "bg-gray-300" : "bg-green-200"
                    }`}
                >
                  <Text
                    className={`text-[12px] font-semibold ${status === "Completed"
                      ? "text-gray-700"
                      : "text-green-700"
                      }`}
                  >
