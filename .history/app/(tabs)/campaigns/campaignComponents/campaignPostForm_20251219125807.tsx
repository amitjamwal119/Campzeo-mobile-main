...
export default function CampaignPostForm({
  ...
  const handleAIGenerateMessage = async () => {
    ...
    try {
      ...
      const response = await generateAIContentApi(requestPayloconst response = await generateAIContentApi(
  {
    prompt: subject,
    context: {
      platform: platform,
  ...
  token // guaranteed non-null here
);


      // AI response has content & variations
      const suggestions = [
        response.content,
        ...
        <View className="flex-1 bg-gray-100 p-4">
          ...
          {showWhatsAppContent && (
            <>
              ...
              {/* AI Suggestions */}
              {loadingAI && (
                <Text style={{ marginVertical: 8 }}>Generating suggestions...</Text>
              )}
              {aiResults.length > 0 && (
