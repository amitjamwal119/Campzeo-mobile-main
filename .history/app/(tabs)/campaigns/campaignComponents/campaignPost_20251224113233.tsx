...
export default function CampaignPost() {
  ...
  const [postData, setPostData] = useState<any | null>(null); // For edit
  ...
  useEffect(() => {
    ...
    const fetchPostDetails = async () => {
      try {
        ...
        setPostData(data); // store fetched post
        if (data?.type) setSelected(data.type); // auto-select platform
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ---------- FORM ---------- */}
        {selected && (!postId || postData) && (
          <View style={{ marginTop: 0, marginBottom: 5 }}>
            <CampaignPostForm
              platform={selected}
              campaignId={campaignId}
              existingPost={postData ?? undefined} // pass fetched post
