...
export default function CampaignPostForm({
  ...
  const handleAIGenerateMessage = async () => {
    ...
    try {
      ...
      const response = await generateAIContentApi({ prompt: subject, platform }, token);
      setAiResults(response.results || ["No suggestions returned."]);
      setLoadingAI(false);
    } catch (error: any) {
      console.error("AI generation error:", error);
      Alert.alert("Error generating AI content", error?.message || "Unknown error");
      setLoadingAI(false);
    }
  };

  const handleChooseFile = () => Alert.alert("📁 File picker will be implemented here.");
