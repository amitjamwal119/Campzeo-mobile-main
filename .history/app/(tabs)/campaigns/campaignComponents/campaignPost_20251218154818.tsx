...
export default function CampaignPost() {
  ...
  const { campaignId, editPost } = route.params as { 
    campaignId: string; 
    editPost?: string; 
  };
  const [selected, setSelected] = useState<string | null>(initialPlatform || null);
    console.log("CampaignPost.tsx received campaignId:", campaignId);

  //     React.useEffect(() => {
  //   if (editPost) {
  //     const post = JSON.parse(editPost);
  //     setSelected(post.type);       // auto-select platform
  //     setEditPostData(post);        // save post for prefill
  //   }
  // }, [editPost]);
