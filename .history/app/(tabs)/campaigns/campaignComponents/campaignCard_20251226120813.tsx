...
export default function CampaignCard({
  ...
  const handleDelete = () => onDelete(campaign);
  const handleAddPost = () => {
      ...
      {isExpanded && (
        <View>
          ...
          <View className="mb-3">
            ...
            <Text className="text-gray-700">{campaign.dates ?? "Date not available"}</Text>
          </View>

          {/* Contacts, Status, Post */}
          <View className="flex-row items-start mt-3 w-full">

            {/* Left: Contacts list */}
            <View className="flex-1">
              <Text className="font-bold text-gray-900 mb-1">Contacts ({contactsCount})</Text>
              {campaign.contacts && campaign.contacts.length > 0 ? (
                campaign.contacts.map((c) => (
                  <Text key={c.id} className="text-gray-700 text-sm">
                    {c.name} ({c.email})
                  </Text>
                ))
              ) : (
                <Text className="text-gray-500 text-sm">No contacts</Text>
              )}
            </View>
            ...
            {/* Right: Post */}
            <View className="flex-1 items-end mt-1">
              {showPostButton && !postButtonTopRight && (
                ...
                >
                  ...
                  <Text className="ml-1.5 text-[12px] text-blue-500 font-semibold">Post</Text>
                </TouchableOpacity>
              )}
            </View>
