  ...
  const fetchCampaigns = async () => {
    try {
      ...
      const mapped = campaignsArray.map((item: any) => {
        ...
        const status = endDate && endDate < today ? "Completed" : "Scheduled";
        ...
        return {
          ...
          posts: item.posts ?? [],
          show: true,
          contactsCount: item._count?.contacts ?? 0, // ✅ correct contacts count
          contacts: [], // API doesn't return full contacts array here
          status,
        };
      });
