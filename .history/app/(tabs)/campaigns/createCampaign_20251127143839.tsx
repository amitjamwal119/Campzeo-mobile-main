...
export default function CreateCampaign({ onCreate }: { onCreate?: (c: Campaign) => void }) {
  const navigation = useNavigation<CreateCampaignScreenProp>();
  const route = useRoute();

  // If campaign is passed via edit
      ...
      <ScrollView className="flex-1 px-6 py-8" keyboardShouldPersistTaps="handled">
        ...
        >
          <Text className="text-white font-semibold text-lg">
            {submitting ? (editingCampaign ? "Updating..." : "Creating...") : editingCampaign ? "Edit Campaign" : "Create Campaign"}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
