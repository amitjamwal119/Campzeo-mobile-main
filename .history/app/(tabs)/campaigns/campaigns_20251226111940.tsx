  ...
  const fetchCampaigns = async () => {
    try {
      ...
      const mapped = campaignsArray.map((item: any) => {
        const startDate = item.startDate?.split("T")[0] ?? null;
        const endDate = item.endDate?.split("T")[0] ?? null;
        const status =
          endDate && new Date(endDate) < today ? "Completed" : "Scheduled";
        ...
        return {
          ...
          posts: item.posts ?? [],
          show: true,
          contactsCount: Array.isArray(item.contacts)
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert("Failed to delete campaign", error?.message || "Unknown error");
          }
        },
      },
    ]);
    ...
    <View className="flex-1 p-4 bg-gray-100">
      ...
      <View className="flex-row items-center mb-4">
        ...
        >
          ...
          <Text className="ml-2" style={{ fontWeight: "600", color: "#3b82f6" }}>
            New
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity onPress={handleShare} className="px-3 py-2 rounded-full bg-green-100 mr-2">
          <Ionicons name="share-social-outline" size={20} color="#16a34a" />
        </TouchableOpacity>

        <TouchableOpacity onPress={toggleFilter} className="flex-row items-center px-3 py-2 rounded-full bg-yellow-100">
      ...
      <FlatList
        ...
        ListEmptyComponent={
          loading ? null : (
            <Text style={{ textAlign: "center", marginTop: 20 }}>No Campaigns Found</Text>
          )
        }
        ListFooterComponent={
          filtered.length > 5 ? (
