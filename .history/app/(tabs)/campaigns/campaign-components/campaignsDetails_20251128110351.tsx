...
export default function CampaignsDetails() {
  ...
  // Handle edit post (navigate to campaignPost.tsx)
  const handleEditPost = (post: any) => {
    router.push({
      pathname: "/campaigns/campaignPost",
      params: { post: JSON.stringify(post) }, // send post data to prefill
    });
  };
        ...
        {tempPosts.length === 0 ? (
          ...
          tempPosts.map((item) => (
            ...
            >
              ...
              {/* Edit & Delete Buttons with light colors */}
              <View className="flex-row mt-3 space-x-3">
                <TouchableOpacity
                  onPress={() => handleEditPost(item)}
                  className="bg-yellow-100 py-2 px-4 rounded-lg flex-1 border border-yellow-300"
                >
                  <Text className="text-yellow-700 text-center font-semibold">Edit</Text>
                </TouchableOpacity>

                <TouchableOpacity
                  onPress={() => handleDeletePost(item.id)}
                  className="bg-red-100 py-2 px-4 rounded-lg flex-1 border border-red-300"
                >
                  <Text className="text-red-700 text-center font-semibold">Delete</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))
