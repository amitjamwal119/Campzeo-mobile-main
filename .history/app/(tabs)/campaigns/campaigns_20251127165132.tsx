...
export default function Campaigns() {
  ...
  // Filter + Search
  let filtered = campaigns.filter((c) => c.details.toLowerCase().includes(search.toLowerCase()));
  ...
  const currentCampaigns = filtered.slice((page - 1) * pageLimit, page * pageLimit);

  // Handlers
  const handleDelete = (c: Campaign) => setCampaigns((prev) => prev.filter((x) => x.id !== c.id));
  const handleCopy = (c: Campaign) => setCampaigns((prev) => [{ ...c, id: prev.length + 1, details: c.details + " (Copy)", posts: [], description: c.description }, ...prev]);
  const handleToggleShow = (c: Campaign) => setCampaigns((prev) => prev.map((x) => (x.id === c.id ? { ...x, show: !x.show } : x)));
  const handleShare = async () => {
    if (!campaigns.length) return;
    const header = "Details\tDates\tDescription\n";
    const message = header + campaigns.map((c) => `${c.details}\t${c.dates}\t${c.description}`).join("\n");
    try { await Share.share({ message }); } catch (e) { console.log(e); }
  };
  const toggleFilter = () => setFilter(filter === "all" ? "show" : filter === "show" ? "hide" : "all");

  // Navigate to CampaignPostForm
  const handleAddPost = (campaign: Campaign) => {
    router.push({
      pathname: "/campaigns/campaign-components/campaignPostForm",
  ...
  return (
    <View className="flex-1 p-4 bg-gray-100">
      ...
      <View className="flex-row items-center mb-4">
        <TouchableOpacity onPress={() => router.push("/campaigns/createCampaign")} className="flex-row items-center justify-center px-3 py-2 rounded-xl bg-blue-100 mr-2">
        ...
        <TouchableOpacity onPress={handleShare} className="px-3 py-2 rounded-xl bg-green-100 mr-2">
          <Ionicons name="share-social" size={20} color="#16a34a" />
        </TouchableOpacity>

        <TouchableOpacity onPress={toggleFilter} className="flex-row items-center px-3 py-2 rounded-xl bg-yellow-100">
          <Ionicons name="funnel-outline" size={20} color="#f59e0b" />
          <Text className="ml-2 font-semibold text-yellow-700">{filter === "all" ? "All" : filter === "show" ? "Show" : "Hide"}</Text>
        </TouchableOpacity>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <CampaignCard
            ...
            showPostButton={true}
            alwaysExpanded={true}
            onPressPost={() => handleAddPost(item)}
          />
        )}
        ListFooterComponent={<Pagination currentPage={page} totalPages={totalPages} onPageChange={setPage} />}
      />
    </View>
  );
}
