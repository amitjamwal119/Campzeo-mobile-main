...
export default function CampaignsDetails() {
  ...
  const handleEditPost = (post: any) => {
    router.push({
      pathname: "/(tabs)/campaigns/campaignComponents/campaignPost",
      params: { postId: String(post.id), campaignId: String(campaign?.id) },
    });
  };
  ...
  const renderPostItem = ({ item }: { item: any }) => {
    ...
    return (
      <View className="bg-white p-4 rounded-xl mb-4 shadow relative">
        ...
        {item.subject ? (
          <Text className="text-lg font-bold mb-2 pr-20" numberOfLines={1} ellipsizeMode="tail">
            {item.subject}
          </Text>
        ) : (
          <View className="flex-row items-center mb-2">
            <Ionicons name="information-circle-outline" size={18} color="#9ca3af" />
            <Text className="ml-2 text-gray-500 italic">No subject available</Text>
          </View>
        )}
        ...
        {item.message ? (
          ...
          <View className="flex-row items-center mb-2">
            <Ionicons name="information-circle-outline" size={18} color="#9ca3af" />
            <Text className="ml-2 text-gray-500 italic">No description available</Text>
          </View>
        )}
        ...
        {item.scheduledPostTime ? (
          ...
          <View className="flex-row items-center mb-2">
            ...
            <Text className="ml-2 text-gray-500 italic">No schedule available</Text>
          </View>
        )}
  ...
  onDelete={(c) => console.log("Delete campaign", c)}
  onEdit={(c) => router.push({
      pathname: "/(tabs)/campaigns/campaignComponents/campaignPost",
      params: { campaignId: c.id.toString() },
  })}
  onCopy={(c) => console.log("Copy", c)}
  onToggleShow={(c) => console.log("Toggle show", c)}
  onPressPost={() => router.push({
