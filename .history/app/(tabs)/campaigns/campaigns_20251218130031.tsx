...
import { ActivityIndicator, Alert, FlatList, Share, TextInput, TouchableOpacity } from "react-native";
...
export default function Campaigns() {
  ...
  const [filter, setFilter] = useState<"show" | "hide" | null>(null); // null = default no text
  ...
  // Toggle between Show / Hide with default no text
  const toggleFilter = () => {
    if (filter === null) setFilter("show");
    else if (filter === "show") setFilter("hide");
    else setFilter(null);
    setVisibleCount(5);
  };
    ...
    <View className="flex-1 p-4 bg-gray-100">
      ...
      <View className="flex-row items-center mb-4">
        ...
        >
          ...
          {filter && (
            <Text className={`ml-2 font-semibold ${filter === "show" ? "text-green-600" : "text-yellow-700"}`}>
