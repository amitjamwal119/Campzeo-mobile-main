...
export default function CampaignsDetails() {
  ...
  const handleEditPost = (campaignId:any ,post: any) => {

    router.push({
      pathname: "/campaigns/campaignComponents/campaignPost",
      params: {  campaignId: String(campaign?.id) ,postId: String(post.id), },
    });
  };
    ...
    return (
      <View className="bg-white p-4 rounded-xl mb-4 shadow relative">
        <View className="absolute top-3 right-3 flex-row space-x-2">
          <TouchableOpacity onPress={() => handleEditPost( campaign?.id,item)} className="mx-1">
